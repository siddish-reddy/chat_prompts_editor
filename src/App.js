...
function Form({ data, onDataChange }) {
  const handleRoleChange = useCallback(
    (index, newRole) => {
      onDataChange(
        data.map((d, i) => (i === index ? { ...d, role: newRole } : d))
      );
    },
    [data, onDataChange]
  );
  ...
  const handleContentChange = useCallback(
    (index, newContent) => {
      onDataChange(
        data.map((d, i) => (i === index ? { ...d, content: newContent } : d))
      );
    },
    [data, onDataChange]
  );
...
function App() {
  ...
  const handleCopy = () => {
    ...
    const dataWithoutEmptyContent = dataWithoutId.filter(
      ({ content }) => content !== ""
    );
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      console.error(error);
      alert(
        "Failed to generate data. Please make sure you have entered a valid OpenAI API key."
      );
    }
    setLoading(false);
  };
  ...
  const handlePaste = async () => {
    try {
      ...
      console.error(error);
      alert(
        "Failed to paste data from clipboard. Please make sure the copied data is a valid JSON."
      );
    }
  };
    ...
    <div className="App min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-10">
        ...
        <div className="mb-4 flex items-center">
          <button
            className="bg-yellow-500 text-white font-bold px-6 py-2 rounded shadow mr-3"
            onClick={handlePaste}
          >
          ...
          <textarea
            ...
            className="border border-gray-300 rounded shadow-sm min-h-[auto] ml-4 p-2 mr-3 h-10 resize-none"
            // style={{ width: '500px' }}
            rows="1"
          />
          model:
          <select
            ...
            className="border text-justify border-gray-300 rounded shadow-sm p-2 ml-3 w-30 mr-2.5 h-fit"
          >
            <option value="gpt-3.5-turbo">GPT 3.5</option>
            <option value="gpt-4-turbo">GPT 4 Turbo</option>
            <option value="gpt-4o">GPT 4 O</option>
            <option value="gpt-4o-mini">GPT 4 O Mini</option>
            <option value="ft:gpt-3.5-turbo-0125:whai:screener-v0:9ieOmBce"> Screener ft</option>
            <option value="ft:gpt-3.5-turbo-0125:whai:l1-v1-23train-7val:9SjwOjaN">
              L1 ft
            </option>
          </select>
          temperature:
          <textarea
            ...
            className="border border-gray-300 rounded shadow-sm min-h-[auto] ml-3 p-2  h-10 resize-none"
            style={{ width: "50px" }}
            rows="1"
          />
        </div>
